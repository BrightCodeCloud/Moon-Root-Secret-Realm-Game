...
- 未激活：深灰石框、内部亮度低于 35%。
- 已激活：青色核心稳定发光，并显示主标签图形。
- 被腐化：橙红核心、边缘有缺口，仍保留原标签符号。
- 季种印记：种子位于闭合根环内，避免与普通掉落种子混淆。
- 根线使用 4–6 px 设计像素宽的分叉线；友方传播为青色连续线，腐化回流为橙红断续线。
- Boss 继承风险必须在 Boss 房图标外显示警戒根冠，不只在侧栏写文字。

对应资源：`res://assets/ui/moonroot-network-ui-atlas.png`。
...
布局参考：`res://assets/ui/mockups/moonroot-network-map-reference.png`。

### 8.6 继续游戏、难度与排行榜

- 继续游戏卡显示关卡、房间进度、角色、武器、种子、难度、有效时间、安全存档状态和网络缩略图。
- 简单、普通、困难必须使用叶片、交叉工具、荆棘冠三种不同剪影；困难卡额外显示第二条 Boss 血条符号。
- 排行榜用独立图标区分死亡、通关、战斗中断和回溯局；被排除出速通榜时显示“交叉沙漏”，不能只降低文字亮度。
- 每关积分与时间使用同一行对照，选中记录后才展开完整构筑，避免主榜信息过密。

布局参考：

- `res://assets/ui/mockups/continue-difficulty-reference.png`
- `res://assets/ui/mockups/run-records-leaderboard-reference.png`

状态图标：`res://assets/ui/challenge-status-atlas.png`。
...
1. 主角完整基础动画。
2. 4 种种子与四阶段生长动画。
3. 4 种普通敌人、1 个精英变体、1 个 Boss。
4. 芽枝杖、月牙镰、日棱喷壶三把武器的四向本体。
5. 首区 16 px 图块集、6–8 个探索房组合、商店、温室和 Boss 房。
6. 战斗 HUD、月根网络地图、三选一卡片、暂停、难度选择、结算和排行榜。
7. 命中、受伤、播种、成熟、收割、根线传播、房间清除七套关键特效。
8. 16 个通用图标、16 个月根网络图标、16 个挑战状态图标和 12 个遗物图标。

每个资产需附：像素尺寸、锚点、碰撞参考、动画帧率、色板索引和导出命名。

### 11.1 交付等级

为避免把概念图误当成最终运行时资源，所有资产标记为：

- **R（Runtime）**：已按固定网格切分，可直接导入动画或 UI。
- **P（Prototype）**：可直接作为原型占位或按明确区域切分，但仍需缩放、九宫格或图集重打包。
- **A（Anchor）**：造型、色板或关键姿势参考，不能直接声明为最终动画或无缝 Tileset。

现有生成资源主要为 P 或 A。正式动画、精确碰撞、无缝边缘和字体授权必须在接入时单独验收。

### 11.2 垂直切片运行时规格

| 资产 | 单帧/单格 | 锚点 | 原型帧率 | 当前准备状态 |
|---|---:|---|---:|---|
| 莱芽 | 32×40 px | 脚底中心 `(16, 35)` | 待机 6 fps，移动 10 fps，动作 12 fps | A：已有主图与关键姿势板 |
| 普通敌人 | 32×32 px | 脚底中心 `(16, 27)` | 移动 8 fps，攻击 10 fps | A：已有静态主图与关键姿势板 |
| 苔冠刺萝卜 | 40×40 px | 脚底中心 `(20, 34)` | 攻击 10 fps | P：已有透明主图 |
| 灯笼南瓜王 | 96×96 px | 脚底中心 `(48, 82)` | 移动 8 fps，招式 12 fps | A：已有主图与关键姿势板 |
| 武器本体 | 32×32 px | 握柄中心 | 随角色动作 | P：三武器四方向图集 |
| 植物 | 40×40 px | 土地中心 `(20, 31)` | 成长切换 + 2 帧成熟循环 | P：四植物四阶段图集 |
| UI 图标 | 24×24 px | 中心 | 状态切换 | P：已有语义图集，需重打包 |
| 土地 Tile | 16×16 px | 左上 | 静态/2 帧覆盖 | A：已有模块板，需手工无缝化 |

动画关键姿势板：

- `res://assets/characters/production/spring-animation-key-pose-reference.png`
- `res://assets/characters/production/lantern-pumpkin-king-key-pose-reference.png`
